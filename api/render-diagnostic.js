...
export default async function handler(req,res){
 ...
 const stages=[]; let browser,tmp; const t0=now(); const mark=(name,extra={})=>stages.push({name,ms:now()-t0,...extra});
 try{
  mark('start',{ffmpeg:!!ffmpegPath}); if(stop==='start')return res.json({ok:true,stages});
  const ep=await chromium.executablePath(); mark('chromium_path',{present:!!ep}); if(stop==='chromium_path')return res.json({ok:true,stages});
  browser=await launchBrowser(); mark('browser_launched'); if(stop==='browser')return res.json({ok:true,stages});
  ...
  mark('page_loaded',{status:response?.status()||0}); if(stop==='page')return res.json({ok:true,stages});
  await page.evaluate(async()=>{if(document.fonts?.ready)await document.fonts.ready;});
  const state=await page.evaluate(()=>({card:!!document.querySelector('.card'),play:!!document.querySelector('#playBtn'),bars:document.querySelectorAll('.bar').length,audio:!!document.querySelector('#audio')?.src})); mark('dom_ready',state); if(stop==='dom')return res.json({ok:true,stages});
  await page.click('#playBtn'); const card=await page.$('.card'); if(!card)throw new Error('card_not_found');
  tmp=await mkdtemp(path.join(os.tmpdir(),'diag-')); for(let i=0;i<3;i++){const time=i*33;await page.evaluate(t=>{document.querySelector('.player')?.classList.add('playing');for(const el of document.querySelectorAll('.bar,.ripple,.disc,.play,.sparkles *'))for(const a of el.getAnimations({subtree:false})){try{a.pause();a.currentTime=t}catch{}}},time);await card.screenshot({type:'jpeg',quality:70,path:path.join(tmp,`frame-${String(i).padStart(3,'0')}.jpg`)});} mark('frames_captured',{frames:3}); if(stop==='frames')return res.json({ok:true,stages});
  await browser.close();browser=null; const mp4=path.join(tmp,'diag.mp4'); await run(ffmpegPath,['-y','-framerate','30','-i',path.join(tmp,'frame-%03d.jpg'),'-c:v','libx264','-preset','ultrafast','-crf','30','-pix_fmt','yuv420p','-movflags','+faststart',mp4]); mark('ffmpeg_complete');
  return res.json({ok:true,stages});
 }catch(e){mark('failed',{error:e instanceof Error?e.message:String(e)});return res.status(500).json({ok:false,stages});}
