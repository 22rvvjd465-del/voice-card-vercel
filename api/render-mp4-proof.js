...
export default async function handler(req,res){
 ...
 try{
  ...
  const barGeom=await page.evaluate(()=>{const card=document.querySelector('.card').getBoundingClientRect();return[...document.querySelectorAll('.bar')].map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);return{x:r.left-card.left,w:r.width,baseY:r.bottom-card.top,color:cs.backgroundColor}})});
  ...
  const filterFile=path.join(tmp,'filters.txt');await writeFile(filterFile,`${crop},fps=15,${drawFilters}`);await run(ffmpegPath,['-y','-framerate',String(targetFps),'-i',path.join(tmp,'frame-%03d.jpg'),'-i',audio,'-t',String(targetDuration),'-filter_script:v',filterFile,'-map','0:v','-map','1:a?','-c:v','libx264','-preset','ultrafast','-crf','28','-pix_fmt','yuv420p','-r','15','-c:a','aac','-b:a','96k','-shortest','-movflags','+faststart',out]);
  const bytes=await readFile(out);res.setHeader('Content-Type','video/mp4');res.setHeader('Content-Length',String(bytes.length));res.setHeader('Cache-Control','no-store');res.setHeader('X-Koephoto-Proof','real-web-player-state-ranges-v18');res.setHeader('X-Koephoto-Frames',String(frames.length));res.setHeader('X-Koephoto-Capture-Duration',duration.toFixed(3));res.setHeader('X-Koephoto-Target-Duration',String(targetDuration));return res.status(200).send(bytes);
