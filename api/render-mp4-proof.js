...
export default async function handler(req,res){
 ...
 try{
  ...
  // Keep the measured Web-player bar states as data only. No extra Puppeteer screenshots.
  // A compact SVG overlay sequence is generated from those exact DOM measurements.
  const barGeom=await page.evaluate(()=>{const card=document.querySelector('.card').getBoundingClientRect();return[...document.querySelectorAll('.bar')].map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);return{x:r.left-card.left+r.width/2,w:r.width,baseY:r.bottom-card.top,rad:cs.borderRadius,background:cs.backgroundColor}})});
  const overlayFps=15,overlayFrames=Math.max(1,Math.ceil(targetDuration*overlayFps));
  const svgDir=path.join(tmp,'wave-svg');await import('node:fs/promises').then(m=>m.mkdir(svgDir,{recursive:true}));
  const esc=s=>String(s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;');
  for(let i=0;i<overlayFrames;i++){const ms=i*1000/overlayFps;let st=barDiag[0],bd=Infinity;for(const q of barDiag){const d=Math.abs(q.t-ms);if(d<bd){bd=d;st=q}}const bars=barGeom.map((g,j)=>{const h=st?.h?.[j]||0,o=st?.o?.[j]??1,shadow=st?.s?.[j]||'none';let filter='';const m=shadow.match(/rgba?\\(([^)]+)\\)\\s+0px\\s+0px\\s+([0-9.]+)px/);if(m){const blur=Math.max(0,parseFloat(m[2])/2);filter=`filter="drop-shadow(0 0 ${blur}px ${esc(g.background)})"`}return `<rect x="${(g.x-g.w/2).toFixed(2)}" y="${(g.baseY-h).toFixed(2)}" width="${g.w.toFixed(2)}" height="${h.toFixed(2)}" rx="${(g.w/2).toFixed(2)}" fill="${esc(g.background)}" opacity="${esc(o)}" ${filter}/>`}).join('');const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${Math.round(rect.w)}" height="${Math.round(rect.h)}" viewBox="0 0 ${rect.w} ${rect.h}"><rect width="100%" height="100%" fill="none"/>${bars}</svg>`;await writeFile(path.join(svgDir,`w-${String(i).padStart(3,'0')}.svg`),svg)}
  ...
  await run(ffmpegPath,['-y','-framerate',String(targetFps),'-i',path.join(tmp,'frame-%03d.jpg'),'-framerate','15','-i',path.join(svgDir,'w-%03d.svg'),'-i',audio,'-t',String(targetDuration),'-filter_complex',`[0:v]${crop},fps=15[base];[1:v]fps=15[wave];[base][wave]overlay=0:0:shortest=1[v]`,'-map','[v]','-map','2:a?','-c:v','libx264','-preset','ultrafast','-crf','28','-pix_fmt','yuv420p','-r','15','-c:a','aac','-b:a','96k','-shortest','-movflags','+faststart',out]);
  const bytes=await readFile(out);res.setHeader('Content-Type','video/mp4');res.setHeader('Content-Length',String(bytes.length));res.setHeader('Cache-Control','no-store');res.setHeader('X-Koephoto-Proof','real-web-player-svg-bars-v14');res.setHeader('X-Koephoto-Frames',String(frames.length));res.setHeader('X-Koephoto-Capture-Duration',duration.toFixed(3));res.setHeader('X-Koephoto-Target-Duration',String(targetDuration));return res.status(200).send(bytes);
