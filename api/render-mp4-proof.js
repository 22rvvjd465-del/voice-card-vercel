...
export default async function handler(req,res){
 ...
 try{
  ...
  // Convert the exact measured DOM bar states to FFmpeg drawbox expressions.
  // No SVG decoder and no additional Puppeteer screenshots are required.
  const barGeom=await page.evaluate(()=>{const card=document.querySelector('.card').getBoundingClientRect();return[...document.querySelectorAll('.bar')].map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);return{x:r.left-card.left,w:r.width,cy:r.top-card.top+r.height/2,color:cs.backgroundColor}})});
  const overlayFps=15,overlayFrames=Math.max(1,Math.ceil(targetDuration*overlayFps));
  const sampled=[];for(let i=0;i<overlayFrames;i++){const ms=i*1000/overlayFps;let st=barDiag[0],bd=Infinity;for(const q of barDiag){const d=Math.abs(q.t-ms);if(d<bd){bd=d;st=q}}sampled.push(st)}
  const rgbhex=s=>{const m=String(s).match(/rgba?\((\d+)\D+(\d+)\D+(\d+)/);return m?'0x'+[m[1],m[2],m[3]].map(x=>(+x).toString(16).padStart(2,'0')).join(''):'0xd6a52d'};
  const states=[];let prev=null;for(let i=0;i<sampled.length;i++){const st=sampled[i];const sig=(st?.h||[]).map(v=>Math.round(v*2)/2).join(',');if(sig!==prev){states.push({start:i,st});prev=sig}}
  const filters=[];for(let k=0;k<states.length;k++){const s=states[k],end=(k+1<states.length?states[k+1].start:overlayFrames)-1;for(let j=0;j<barGeom.length;j++){const g=barGeom[j],h=s.st?.h?.[j]||0,y=g.cy-h/2;filters.push(`drawbox=x=${g.x.toFixed(2)}:y=${y.toFixed(2)}:w=${g.w.toFixed(2)}:h=${h.toFixed(2)}:color=0xd6a52d@0.92:t=fill:enable='between(n\\,${s.start}\\,${end})'`)}}const drawFilters=filters.join(',');console.log('KOEPHOTO_STATE_DIAG',JSON.stringify({states:states.length,filters:filters.length,overlayFrames}));
  ...
  const waveBounds=await page.evaluate(()=>{const card=document.querySelector('.card').getBoundingClientRect(),bs=[...document.querySelectorAll('.bar')].map(b=>b.getBoundingClientRect());const x=Math.min(...bs.map(r=>r.left))-card.left-5,r=Math.max(...bs.map(r=>r.right))-card.left+5,y=Math.min(...bs.map(r=>r.top))-card.top-8,b=Math.max(...bs.map(r=>r.bottom))-card.top+8;return{x,y,w:r-x,h:b-y}});const cleanColor='0xfffbf2';const filterFile=path.join(tmp,'filters.txt');await writeFile(filterFile,`${crop},fps=15,drawbox=x=${waveBounds.x.toFixed(2)}:y=${waveBounds.y.toFixed(2)}:w=${waveBounds.w.toFixed(2)}:h=${waveBounds.h.toFixed(2)}:color=${cleanColor}:t=fill,${drawFilters}`);await run(ffmpegPath,['-y','-framerate',String(targetFps),'-i',path.join(tmp,'frame-%03d.jpg'),'-i',audio,'-t',String(targetDuration),'-filter_script:v',filterFile,'-map','0:v','-map','1:a?','-c:v','libx264','-preset','ultrafast','-crf','28','-pix_fmt','yuv420p','-r','15','-c:a','aac','-b:a','96k','-shortest','-movflags','+faststart',out]);
  const bytes=await readFile(out);res.setHeader('Content-Type','video/mp4');res.setHeader('Content-Length',String(bytes.length));res.setHeader('Cache-Control','no-store');res.setHeader('X-Koephoto-Proof','real-web-player-clean-gold-bars-v19');res.setHeader('X-Koephoto-Frames',String(frames.length));res.setHeader('X-Koephoto-Capture-Duration',duration.toFixed(3));res.setHeader('X-Koephoto-Target-Duration',String(targetDuration));return res.status(200).send(bytes);
