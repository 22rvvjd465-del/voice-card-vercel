...
export default async function handler(req,res){
 ...
 try{
  ...
  const expr=(j,key)=>sampled.map((st,i)=>`if(eq(n\\,${i})\\,${key==='h'?(st?.h?.[j]||0).toFixed(2):(barGeom[j].baseY-(st?.h?.[j]||0)).toFixed(2)}\\,`).join('')+'0'+')'.repeat(sampled.length);
  const drawFilters=barGeom.map((g,j)=>`drawbox=x=${g.x.toFixed(2)}:y='${expr(j,'y')}':w=${g.w.toFixed(2)}:h='${expr(j,'h')}':color=${rgbhex(g.color)}:t=fill`).join(',');
  ...
  const bytes=await readFile(out);res.setHeader('Content-Type','video/mp4');res.setHeader('Content-Length',String(bytes.length));res.setHeader('Cache-Control','no-store');res.setHeader('X-Koephoto-Proof','real-web-player-filter-script-v17');res.setHeader('X-Koephoto-Frames',String(frames.length));res.setHeader('X-Koephoto-Capture-Duration',duration.toFixed(3));res.setHeader('X-Koephoto-Target-Duration',String(targetDuration));return res.status(200).send(bytes);
