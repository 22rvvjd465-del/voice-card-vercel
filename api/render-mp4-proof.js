...
async function run(cmd,args){return new Promise((resolve,reject)=>{const p=spawn(cmd,args,{stdio:['ignore','ignore','pipe']});let err='';p.stderr.on('data',d=>err+=d);p.on('error',reject);p.on('close',c=>c===0?resolve():reject(new Error(`ffmpeg_${c}: ${err.slice(-1200)}`)))})}
...
export default async function handler(req,res){
 ...
 try{
  ...
  await page.evaluate(async()=>{if(document.fonts?.ready)await Promise.race([document.fonts.ready,new Promise(r=>setTimeout(r,1800))]); const btn=document.querySelector('#playBtn'); if(btn){btn.setAttribute('aria-label','音声を一時停止');btn.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 5h4v14H7zM13 5h4v14h-4z"/></svg>';}});
  const card=await page.$('.card'); if(!card)throw new Error('card_not_found');
  const pause=path.join(tmp,'pause.jpg'); await card.screenshot({type:'jpeg',quality:90,path:pause});
